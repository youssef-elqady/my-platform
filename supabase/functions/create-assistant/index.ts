...
const configuredOrigins = (Deno.env.get("APP_ORIGIN") ?? "").split(",").map((v) => v.trim()).filter(Boolean);
const corsOrigin = (origin: string) => configuredOrigins.length === 0 ? "*" : (configuredOrigins.includes(origin) ? origin : configuredOrigins[0]);
const headers = (origin: string) => ({ "Access-Control-Allow-Origin": corsOrigin(origin), "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type", "Access-Control-Allow-Methods": "POST, OPTIONS", "Vary": "Origin", "Content-Type": "application/json" });
